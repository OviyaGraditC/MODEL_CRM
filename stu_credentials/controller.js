    ...
    if (username !== "" && psd !== "") {
      fetch("https://retoolapi.dev/5Pzpcy/student_login", {
        ...
        .then((data) => {
          ...
          account_data = accounts.filter((account) => {
            return account.username == username;
          });
          ...
          if (account_data && account_data.length == 0) {
            ...
            account_data[0].username == username &&
            account_data[0].psd == psd
          ) {
            window.location.href =
              "../student_home_page/student_home_page.html";
          } else
            document.getElementById("confirm_msg").innerHTML =
              "Error: Data Not Found..!";
        });
    ...
    fetch("https://retoolapi.dev/5Pzpcy/student_login", {
      ...
      .then((data) => {
        ...
        if (username !== "" && psd !== "" && cfmpsd !== "") {
            ...
            if (psd == cfmpsd) {
              ...
              fetch("https://retoolapi.dev/5Pzpcy/student_login", {
                ...
                .then((data) => {
                  ...
                  document.getElementById("confirm_msg").innerHTML =
                    "Automatically Page will be redirected to Login Page";
                  setInterval(() => {
          ...
          } else confirm_msg.innerHTML = "Error : Username Already Exist..";
        } else {
...
function updateAccount(accountId) {
  get_registered_stu_details();
  let update_columns =
    '<input type="text" class="update_login_input" name="idno" id="update_idno" placeholder="idno" readonly><input type="text" class="update_login_input" name="username" id="update_username" placeholder="Enter username"><input type="text" class="update_login_input" name="psd" id="updated_psd" placeholder="Enter password"><button id="update_psd" class="btn btn-success button_top1" onclick="updateAcc()">Update</button>';
...
function updateAcc() {
  ...
  let apiUrl =
    "https://retoolapi.dev/5Pzpcy/student_login/" + accountIdForUpdate;
  fetch(apiUrl, {
    method: "PUT",
    body: JSON.stringify(account),
    headers: { "Content-Type": "application/json; charset=UTF-8" },
...
function displayData(accounts) {
  ...
  var result =
    "<b>" +
    '<th style="background-color: #0e91e3;">' +
    "Id No" +
    "</th>" +
    '<th style="background-color: #0e91e3;">' +
    "Username" +
  ...
  document.getElementById("accounts").innerHTML =
    '<div class="update_column_box">' + result + "</div>";
