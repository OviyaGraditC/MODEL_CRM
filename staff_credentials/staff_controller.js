    ...
    .then((data) => {
      ...
      if (psd == cfmpsd) {
        ...
        fetch(apiUrl, {
          ...
          .then((data) => {
            console.log(data);
            document.getElementById("resetpsd_confirm_msg").innerHTML =
    ...
    if (username !== "" && psd !== "") {
      fetch("https://retoolapi.dev/HOcUqC/emp_data", {
        ...
        .then((data) => {
          ...
          let account_data = accounts.filter((account) => {
            return account.username == username;
          })[0];

          if (account_data.username == username && account_data.psd == psd) {
            window.location.href =
              "../staff_home_page/staff_home_page.html";
          } else
      ...
      .then((data) => {
        ...
        for (let i = 0; i < accounts.length; i++) {
          if (accounts[i].username == username) {
            ...
            break;
          } else account_duplicate_data = false;
        }
...
function updateAccount(accountId) {
  ...
  fetch("https://retoolapi.dev/HOcUqC/emp_data/" + accountId, {
    ...
    .then((data) => {
      ...
      let update_columns =
        '<input type="text" class="update_login_input" name="idno" id="update_idno" placeholder="idno" readonly><input type="text" class="update_login_input" name="username" id="update_username" placeholder="Enter username"><input type="text" class="update_login_input" name="psd" id="updated_psd" placeholder="Enter password"><input type="text" class="update_login_input" name="update_staff_name" id="update_staff_name" placeholder="Enter staff name"><input type="text" class="update_login_input" name="update_staff_location" id="update_staff_location" placeholder="Enter staff location"><input type="text" class="update_login_input" name="update_salary" id="update_salary" placeholder="Enter staff salary"><input type="text" class="update_login_input" name="update_designation" id="update_designation" placeholder="Enter designation"><input type="text" class="update_login_input" name="update_contactnumber" id="update_contactnumber" placeholder="Enter contact number"><input type="text" class="update_login_input" name="update_emp_id" id="update_emp_id" placeholder="Enter emp_id"><button id="update_psd" class="btn btn-success button_top1" onclick="updateAcc()">Update</button>';
...
function displayData(accounts) {
  ...
  for (let account of accounts) {
    ...
    result +=
      "<td>" +
      '<button type="button" style="width:auto; height:auto; font-size:15px;" class="btn btn-warning" onclick="updateAccount(' +
      account.id +
      ')">Update</button>' +
      "</td>";
    result +=
      "<td>" +
      '<button type="button" style="width:auto; height:auto; font-size:15px;" class="btn btn-danger" onclick="delete_registered_stu_details(' +
      account.id +
      ')">Delete</button>' +
      "</td>" +
      "</tr>";
  ...
  document.getElementById("staff_update_columns").innerHTML =
    '<div class="update_column_box">' + result + "</table>" + "</div>";
